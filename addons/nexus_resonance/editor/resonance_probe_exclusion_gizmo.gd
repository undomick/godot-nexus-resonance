...
## Editor gizmo for ResonanceProbeExclusion (orange box + size handles).
...
func _get_gizmo_name() -> String:
	return UIStrings.GIZMO_PROBE_EXCLUSION_CLASS


func _has_gizmo(node: Node) -> bool:
	return node.is_class(UIStrings.GIZMO_PROBE_EXCLUSION_CLASS)


func _init() -> void:
	var sid := str(get_instance_id())
	_mat_main = "nexus_probe_excl_main_" + sid
	_mat_handles = "nexus_probe_excl_handles_" + sid
	_mat_icon = "nexus_probe_excl_icon_" + sid
	create_material(_mat_main, Color(1.0, 0.45, 0.15))
...
func _ensure_icon_material(_gizmo: EditorNode3DGizmo) -> void:
	...
	if FileAccess.file_exists(UIStrings.ICON_PROBE_EXCLUSION_GIZMO):
		icon_tex = load(UIStrings.ICON_PROBE_EXCLUSION_GIZMO) as Texture2D
	if not icon_tex and fallback_icon:
		icon_tex = fallback_icon
	if not icon_tex:
		var gui = EditorInterface.get_base_control()
		if gui:
			icon_tex = gui.get_theme_icon("Stop", "EditorIcons")
	if icon_tex:
		# Orange modulate so the billboard matches the exclusion volume wire color.
		create_icon_material(_mat_icon, icon_tex, false, Color(1.0, 0.45, 0.15))
...
func _redraw(gizmo: EditorNode3DGizmo) -> void:
	...
	if node.has_method("is_enabled") and not node.is_enabled():
		return
...
func _commit_handle(
	...
	if cancel:
		...
		if undo_redo != null and changed:
			undo_redo.create_action(UIStrings.UNDO_CHANGE_PROBE_EXCLUSION_SIZE)
