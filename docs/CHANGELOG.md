...
- **Inner stream access** - `get_inner_stream()` / `get_inner_stream_playback()` on `ResonancePlayer` for VOIP, mic, and generator streams. ([#15](https://github.com/undomick/godot-nexus-resonance/pull/15) by [realpoke](https://github.com/realpoke))
- **Re-Export Static Scenes In All Scenes In Build** - Tools menu entry walks the main scene dependency tree and re-exports scenes that already have a `ResonanceStaticScene` (no auto-create; first-time pack via Export Active Scene).
- **Distance attenuation controls** - Player Config: master on/off, clearer Distance inspector (Mode above Min/Max; Max grayed out for Inverse), optional distance curve on reflections, `occlusion_radius` name for the source radius, plus `set_inputs` / `get_outputs` on `ResonancePlayer`.
- **Rebuild static geometry at runtime** - Export and hot-swap baked static scenes while the game runs (`ResonanceRuntimeExporter`; optional probe bake after reload).
- **Sample Reverb Here** - Reverb Data Point inspector shows readable RT60 and energy; gizmo turns green when a sample succeeded (analysis only).

### Changed

- **Runtime material swap via** `iplStaticMeshSetMaterial` - `ResonanceGeometry.set_material` updates existing static meshes in-place once registered (dynamic path and static path without a loaded `ResonanceStaticScene`). Avoids mesh rebuild and Embree reset on runtime material changes. StaticScene-covered geometry still requires re-export.
- **Static batch export menu** - Removed "Export All Open Scenes"; **Re-Export Static Scenes In All Scenes In Build** is the only batch static path and skips scenes without an existing `ResonanceStaticScene`.
- **Pathing live toggle** - `pathing_enabled`, `pathing_normalize_eq`, `path_validation_enabled`, and `find_alternate_paths` apply without tearing down Steam Audio (fixes dry-through-walls / click on disable). Enabling reinits only if the simulator was created without pathing; `pathing_num_samples` still reinits. Leftover baked pathing while runtime pathing is off is intentional (old warning removed).
- **Pathing quality on RuntimeConfig** - `pathing_vis_range`, `pathing_path_range`, `pathing_vis_radius`, `pathing_vis_threshold`, and `pathing_num_samples` live on `ResonanceRuntimeConfig` for bake and runtime. Defaults 1000 / 1000 / 1.0 / 0.1 / 4. Vis* are live-patchable; path range is bake-only. **Breaking:** BakeConfig pathing quality fields, old defaults (500/100/0.5), and `bake_pathing_num_samples` no longer apply.
- **Baked reflections simplified** - Removed `reflections_sampling_mode`, `apply_occlusion_to_baked_reflections`, `reverb_transmission_amount` and `reverb_influence_radius`.
- **Audio engine settings removed** - Sample Rate Override and Audio Frame Size dropped from `ResonanceRuntimeConfig`. Steam Audio uses Godot mix rate and an Auto frame size from Project Settings latency.
- **Ambisonics: realtime vs bake** - Playback uses `realtime_ambisonic_order` (legacy `ambisonic_order`); bake uses `bake_ambisonic_order` / per-volume `ResonanceBakeConfig.bake_ambisonics_order`. Pathing on/off for bakes follows RuntimeConfig (BakeConfig fields kept for `.tres`, hidden). On mismatch: one aggregated warning, playback `min(baked, realtime)`, batch kept; missing pathing skips pathing only.
- **Bake settings from ResonanceRuntime** - Reflections bake uses `ResonanceRuntime.reflection_type` (TAN → Convolution). STATICSOURCE / STATICLISTENER passes follow `bake_sources` / `bake_listeners` on the volume. Matching BakeConfig fields stay for `.tres` compatibility but are hidden and no longer gate the bake. Type mismatch: red gizmos, editor warning, skip at play with rebake hint.
- **Default export format Binary (.res)** - New projects default `resonance_asset_format` / `probe_data_format` to Binary; existing Text settings kept.
- **Mix mute / wet drain** - Mix levels mute DSP only and no longer clear simulation flags. `enable_direct` / `enable_reverb` follow `output_`* (not Mix ≤ 0). Convolution/path still apply at mix 0 so Overlap-Save drains; unmute ramps from silence.
- **Pathing distance** - Occluded pathing loudness uses baked probe-to-probe path length; LOS still uses Euclidean 1/d in path SH. Pathing wet does not multiply Direct playback attenuation.
- **Debug draw** - F3 no longer draws occlusion lines, reflection rays, or pathing segments.
- **Single IR duration setting** - Removed `max_reverb_duration`. `realtime_simulation_duration` is now the sole source of truth for IR duration. Previously the convolution effect IR buffer came from `max_reverb_duration` while simulation used `realtime_simulation_duration`, and Apply clamped the sim IR to the smaller allocation - so raising simulation duration past `max_reverb_duration` had no audible effect. Effect IR allocation, simulator `maxDuration`, EOS tail-drain grace, and soft-stop watchdog now all use `realtime_simulation_duration` (0.1-10.0 s). Breaking: scripts/configs that read `ResonanceRuntimeConfig.max_reverb_duration` must use `realtime_simulation_duration`.
- **Data-driven Custom-scene materials** - The Godot Physics scene bridge loads `ResonanceMaterial` `.tres`/`.res` from configurable search paths (default `res://addons/nexus_resonance/materials/`, Project Setting `nexus/resonance/physics/material_search_paths`). Colliders can assign a resource via `resonance_physics_material` meta or a preset basename via `resonance_physics_material_preset`. Call `ResonanceServer.refresh_physics_materials()` after editing presets at runtime. Replaces hardcoded physics preset tables.

### Removed

- `bake_probes()` **on Probe Volume** - Use the inspector **Bake** button, `ResonanceBakeRunner`, or `ResonanceRuntimeBaker` instead.

### Fixed

- **Probe grid influence radius** - Manual grid used `radius = 1 m` instead of spacing; occluded pathing loudness could freeze. **Rebake probe volumes** after updating.
- **Wet continuity** - Convolution/TAN apply every block with the TripleBuffer IR handle; EOS `GetTail` no longer aborts on a stale cache epoch. Pathing/reverb no longer drop on a missed sim tick. Occlusion scales Direct, not convolution wet (`wet_occlusion_factor` stays 1).
- **Stop / stream end / reverb tail** - Finished streams release the voice after tail drain ([#16](https://github.com/undomick/godot-nexus-resonance/pull/16) by [realpoke](https://github.com/realpoke)). Stop no longer hangs, clicks, or cuts the wet tail short.
- **Shutdown / audio-thread safety** - Quit no longer waits forever on the reverb mixer; Ambisonic IPL no longer rebuilds on the audio thread; mixer teardown no longer use-after-free.
- **Listener without ResonanceListener** - Camera pose is used and marked valid so audio is not simulated at the origin.
- **Custom transmission rays** - Godot Custom [code]closestHit[/code] uses the same start-t and [code]hit_from_inside(false)[/code] as occlusion [code]anyHit[/code], so a single wall no longer picks an inside/floor hit as the transmission surface.
- **Occlusion after geometry change** - Direct/occlusion runs after scene commit so newly spawned meshes occlude on the same tick.
- **Bake / pathing stale wet** - Failed bakes report failure; pathing no longer keeps a stale wet mix.
- **Batch static export** - **Re-Export Static Scenes In All Scenes In Build** and Export Active Scene share the same export SSOT (asset path + hash on `ResonanceStaticScene`, skip when unchanged) and are blocked while the editor is playing.
- **Per-player reverb binaural** - `reverb_binaural_override` now affects the local wet mix.
- **Start reverb delay / quit freeze vs Steam Audio** - Cold start coalesces geometry commits until tree settle (no worker wake / tick schedule while pending), then triggers the first IR immediately (no interval wait; first RunReflections is not deferred after a heavy scene commit). Deferred warmup no longer re-arms the spatial gate after triangles are already registered. On quit, `ResonanceRuntime` arms teardown from Window `close_requested` (and last-runtime `tree_exiting`): mute mix, stop new heavy sims, join the worker before world Geometry/Probe `EXIT_TREE`. Live Geometry/Probe despawn only clears that node's meshes/batches - it does not shut the server down - so spawn, despawn, and `set_material` keep working without reinit. Dynamic geometry skips per-node global `iplSceneCommit` only after quit teardown is already armed. Dynamic transform flush respects the same cold-start coalesce wake gate as geometry notify.
- **Runtime export + probe bake** - Probe bake after static rebuild waits for the next frame (and `export_static_async` can await both). Failed baked point/probe queries return a clear error instead of a partial result.
...
- **ResonanceRuntime.prepare_for_shutdown** - Hard-stops players and detaches `ResonanceAudioEffect` so AudioServer can retire them before GDExtension deinit. Call a few frames before `SceneTree.quit` to avoid an exit crash (issue #13).
- **Incremental static packs** - `add_or_replace_static_pack` / `remove_static_pack` and multi-pack bake via `set_bake_static_scenes_from_assets`; spawn/despawn no longer rebuilds every pack.
- **ResonanceProbeExclusion** - Child box under `ResonanceProbeVolume` that discards probe candidates inside its OBB before bake.
- **ResonanceProbeVolume bake UX** - Auto-creates `bake_config` when empty; `scan_targets` / **Update Targets**; runtime add/remove bake source and listener helpers.
...
- **Export / bake data layout** - Project Settings key renamed to **Resonance Asset Format**; default output is `res://resonance_data/` with `statics/`, `dynamics/`, and `batches/` subfolders (legacy paths migrated on plugin load).
- **Export Active Scene / static ownership** - Nested and instanced `ResonanceStaticScene` handling; bake feeds every section pack; clearer parent-export messages.
- **ResonancePlayer** - ASP3D transport + `ResonancePlayerConfig`; `volume_db` / `max_db` as pre-Steam source loudness; convolution mixer feed uses `reflections_mix_level` only.
- **ResonanceProbeVolume** - `bake_probes()` / `bake_probes_with_floor_points()` deprecated; inspector **Bake** / `ResonanceBakeRunner.run_bake` is the supported pipeline.
...
- **ResonanceRuntime** - Primary-runtime tick ownership; static pack spawn/despawn; scene handoff without full reinit.
- **ResonancePlayer soft-stop** - Completes host `stop()` after tail drain; process enabled with `player_config` for volume cache and soft-stop.
- **Baker / geometry** - Isolated temp `IPLScene` for probe bake (no long `simulation_mutex` hold); bake thread result no longer discarded; safer mesh rebuild and worker-thread logging.
...
- **Reverb drop/pop at the end of short audio clips** - Mixer return now avoids pulling the shared reflection mixer when no sources have fed it in the current audio quantum (Godot scheduling + mixer mutex ordering edge case). This prevents an audible wet-level dip right as one-shot clips end.
...
- **Pathing wet level** - No extra multiply by `reverb_pathing_attenuation` (distance is already in baked path SH). `pathing_mix_level` ramps the **mono input** before `iplPathEffectApply`, not the stereo output.
