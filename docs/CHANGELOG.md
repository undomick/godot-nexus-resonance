...
- **ResonancePlayer** - ASP3D transport + `ResonancePlayerConfig`; `volume_db` / `max_db` as pre-Steam source loudness; convolution mixer feed uses `reflections_mix_level` only (Unity parity).
